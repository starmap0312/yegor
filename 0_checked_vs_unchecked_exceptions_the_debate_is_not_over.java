...
// 1) hiding the fact that a method may fail
// 2) the method may be too complex and want to keep some exceptions "hidden" (i.e. unchecked)
...
// 1) unlike unchecked exceptions, we can't ignore failures
//    need to try/catch somewhere
// 2) every method is either "safe" (throws nothing) or "unsafe" (throws Exception)
...
// 3) use only Exception without any sub-types
//    we don't need multiple exception types, because we don't use exceptions to control flow
// 4) never recover from exceptions
//    i.e. never catch without rethrowing (no fail slow)
